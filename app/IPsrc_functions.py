...
def plot_pie(df, col):
    pie = px.pie(df.head(10), values=col, names='IPsrc', title='Total number of requests {} by IPsrc'.format(col))
    return pie


def plot_scatter(df, col1, col2):
    scatter = px.scatter(df, x=col1, y=col2, title='Total number of requests by IPsrc', 
...
def plot_pie_ratio(df, col):
    pie = px.pie(df, values=col, names='class', title='Ratio of denied requests by IPsrc', category_orders={'class': ['90-100%', '80-90%', '70-80%', '60-70%', '50-60%', '40-50%', '30-40%', '20-30%', '10-20%', '0-10%']})
    return pie
